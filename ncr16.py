...
# Function to save mesh and field data to a VTK file
def save_to_vtk(filename, meshes, momentum_exchange, mass_flow):
    for name, mesh in meshes.items():
        points = mesh.vertices
        cells = {"triangle": mesh.faces}

        if name == 'walls':
            cell_data = {"momentum_exchange": [np.linalg.norm(momentum_exchange, axis=1)]}
        elif name == 'outlet':
...
save_to_vtk("mesh_data", meshes, momentum_exchange, mass_flow)

print("Data for ParaView visualization has been saved to 'mesh_data_walls.vtk' and 'mesh_data_outlet.vtk'.")
