...
# Generate random rays with origins at the "inlet.stl" surfaces
num_rays = 8450
...
# Count the number of origins generated at each triangle of the "inlet.stl" mesh
origins_per_triangle = np.zeros(len(meshes['inlet'].faces))
for idx in ray_origins_indices:
    origins_per_triangle[idx] += 1

print("\nNumber of origins per triangle on 'inlet.stl':")
...
def save_to_vtk(filename, meshes, momentum_exchange, mass_flow, origins_per_triangle):
    for name, mesh in meshes.items():
        ...
        meshio.write_points_cells(
            f"vtk/{filename}_{name}16.5.vtk",
            points,
            cells,
            cell_data=cell_data
        )
