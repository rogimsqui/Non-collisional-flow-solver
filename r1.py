...
#random_point = [random_point_in_mesh(mesh) for _ in range(1)]
random_point = random_point_in_mesh(mesh)

# Function to plot the mesh and random point
def plot_mesh_and_point(mesh, random_point):
    ...
    ax.scatter(*random_point, color='r', s=50, label='Random Point')
    ...
    ax.set_zlabel('Z')
    ax.legend()
    plt.title('Mesh with Random Point')
    plt.show()

# Plot the mesh and the random point
plot_mesh_and_point(mesh, random_point)
