...
def plot_mesh_and_rays(mesh, original_origins, original_directions, reflection_origins, reflection_directions, intersections=None, reflections=None, intersection_points=None):
    ...
    for origin, direction in zip(original_origins, original_directions):
        ...
        ax.plot([origin[0], ray_end[0]], [origin[1], ray_end[1]], [origin[2], ray_end[2]], 'r-', linewidth=2.5)

    # Plot reflected rays (as blue segments connecting intersection points)
    if intersection_points is not None and len(intersection_points) > 0:
        # Flatten the list of intersection points for easier processing
        intersection_points = np.array(intersection_points)

        # Draw lines connecting the intersection points
        num_points = len(intersection_points)
        for i in range(num_points - 1):
            ax.plot([intersection_points[i][0], intersection_points[i + 1][0]],
                    [intersection_points[i][1], intersection_points[i + 1][1]],
                    [intersection_points[i][2], intersection_points[i + 1][2]], 'b-', linewidth=0.5)
...
ray_origins = np.array([random_point_in_mesh(inlet_mesh) for _ in range(1)])
...
random_directions = np.random.normal(size=(1, 3))
...
reflection_counts = count_reflections_per_triangle(meshes['walls'], all_origins, all_directions, intersection_indices, reflections)
...
crossing_counts = count_crossings_per_triangle(meshes['outlet'], all_origins, all_directions)
