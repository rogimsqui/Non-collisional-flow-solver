...
def trace_rays(meshes, ray_origins, ray_directions, max_reflections=2):
    all_origins = []
    all_directions = []
    reflections = []
    intersection_points = []
...
def save_to_vtk(filename, meshes, momentum_exchange, mass_flow, origins_per_triangle):
    for name, mesh in meshes.items():
        ...
        meshio.write_points_cells(
            f"vtk/{filename}_{name}.17.6.vtk",
            points,
            cells,
            cell_data=cell_data,
